...
The parser applies rules in a determined order (a.k.a. precedence) to avoid any
ambiguity. The elements take their precedence in following order:
...
If the `First Match` is a container block start token, the `Residual Before` is
known to be a solid block and can be parsed with {Parser#__parseSolidBlocks}.
...
'> ' for blockquotes or correct level of indentation for list items). Delimiters
are stripped before the contents are aggregated into one new block for later
parsing. A new line without a container block delimiter can either be the end
of current container block or should be added to the container accroding to
'laziness' rule. The parsing is not complete until either the end of container
block or the end of the document is encountered.
...
class Parser
  ...
  # Parses Markdown source into AST
  ...
  # @private
  # Parses the source starting from given offset and tries to find the first
  ...
  # @private
  # Parses the specified document region as a solid block
  ...
  # @private
  # Parses the specified document region as a fluid block
